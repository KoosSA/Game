...
public class ChunkGenerator {
	...
	public static Chunk generateChunk(int xpos, int zpos, int chunkLength) {
		...
		if (f.exists()) {
			return loadChunkFromFile(name, chunkLength);
		}
		return genNewChunk(xpos, zpos, name, chunkLength);
	}
	
	private static Chunk loadChunkFromFile(String name, int chunkLength) {
		...
		return new Chunk(renderArr, cd.heights, cd.transform, cd.indices.length, chunkLength);
	}
	
	private static Chunk genNewChunk(int xpos, int zpos, String name, int chunkLength) {
		...
		transform.setPosition(xpos * chunkLength, 0, zpos * chunkLength);
		...
		int[] renderArr = loadModelData(true, cd.vertices, cd.normals, cd.indices);
		return new Chunk(renderArr, cd.heights, transform, inds.size(), chunkLength);
	}
